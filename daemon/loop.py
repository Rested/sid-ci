...
) -> Iterator[sid_pb2.Repo]:
    return stub.GetRepos(sid_pb2.Repo(
        enabled=True
    ))


def check_for_changes_to_enabled_repos(repos: Iterator[sid_pb2.Repo]) -> List[Repository]:
    changed_repos = []
    unchanged_repos = []
    no_repos = True
    for repo in repos:
    ...
    if no_repos:
        logger.info("No enabled repos to check...")
...
def add_changed_repos_to_queue(
    ...
    for repo in changed_repos:
        # send to the server
        timestamp = Timestamp()
        logger.info("Adding job: %s:%s", repo.ssh_url, repo.target_ref)
...
def loop():
    ...
    while True:
        ...
        try:
            ...
            logger.info("Connected to sid server on %s", settings.sid_server_dsn)
        except grpc.RpcError:
            logger.debug("Failed to connect to server", exc_info=True)
            if server_waiting_for > settings.sid_server_timeout:
                logger.error("Connection to sid server (%s) timed out after %d seconds", settings.sid_server_dsn,
                             settings.sid_server_timeout)
                break
            time.sleep(1)
            continue
        logger.info("Getting enabled repos")
        enabled_repos = get_enabled_repositories(stub)
        logger.info("Checking for changes in enabled repos")
        changed_repos = check_for_changes_to_enabled_repos(enabled_repos)
        add_changed_repos_to_queue(changed_repos, stub)
        logger.info(
